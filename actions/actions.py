...
class BaseSwiperAction(Action, ABC):
    ...
    async def run(
        ...
        logger.info('BEGIN ACTION RUN: %s (tracker.sender_id=%r)', self.name(), tracker.sender_id)
        ...
        logger.info('END ACTION RUN: %s (tracker.sender_id=%r)', self.name(), tracker.sender_id)
        return events
